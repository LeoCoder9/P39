...
function setup() {
  ...
  knight = createSprite(100, 300, 150, 150)
  ...
  ground = createSprite(800, 1280 , width, 150)
...
function draw() {
  ...
  if (gamestate === "PLAY") {
    ...
    */
    ...
    knight.velocityY = knight.velocityY +6
  ...
  drawSprites()
    ...
    textSize(55)
    fill("Black")
    text("WAVE " + wave + ":", camera.x - 100, camera.y - 300)


    textSize(50)
    fill("red")
    text("Kills: " + score, camera.x + 150, camera.y - 300)

    textSize(40)
    fill("black")
    text("Time: " + time, camera.x - 700, camera.y - 300)

    textSize(40)
    fill("black")
    text("Lives: " + lives, camera.x + 600, camera.y - 300)
  ...
  if (gamestate === "END") {
    ...
    fill("RED")
    textFont("bold")
    text("YOUR STATS:", camera.x - 260, camera.y  - 100)
    fill("white")
    textSize(20)
    text("YOUR LIVES RAN OUT!", camera.x - 50, camera.y  - 200)
    fill("white")
    text("YOUR KILLS:" + score, camera.x - 250, camera.y  - 50)
    text("YOUR TIME:" + time,  camera.x - 250, camera.y  + 20)

    textSize(100)
    fill("red")
    text("GAME OVER!", camera.x - 280, camera.y  - 250)
    textSize(50)
    fill("white")
    text("To RETRY press 'R' , Good Luck!", camera.x - 280, camera.y + 150)
...
function monsters2() {
  if (frameCount % 55 === 0) {
    monster2 = createSprite(1600, 1170, 10, 10);
